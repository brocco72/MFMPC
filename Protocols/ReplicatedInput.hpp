...
inline void MultiReplicatedInput<T>::add_mine(const typename T::open_type& input, int n_bits)
{
    ...
    shares.push_back({});
    T& my_share = shares.back();
    my_share[0].randomize(secure_prng, n_bits);
    my_share[1].randomize(secure_prng, n_bits);
//    cout << my_share[0] << " " << my_share[1] << endl;
    typename T::value_type next_share = input - my_share[0];
    next_share.pack(os[0], n_bits);
    my_share[1].pack(os[0], n_bits);
    typename T::value_type prev_share = input - my_share[1];
    prev_share.pack(os[1], n_bits);
    my_share[0].pack(os[1], n_bits);
    this->values_input++;
}
