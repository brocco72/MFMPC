...
int main(int argc, const char *argv[])
{
  ...
  while ( tokens[0] != "XOR2" && tokens[0] != "OR2" && tokens[0] != "INV" && tokens[0] != "AND2" && tokens[0] != "AND3" && tokens[0] != "AND4" && tokens[0] != "assign");

  // Now add the constant wires
  // stringstream w0, w1;
  // w0 << "1'b0";
  // w1 << "1'b1";
  ...
  for (unsigned int i= 0; i < gates.size(); i++)
    {
      ...
      if (gates[i][0] == "INV")
      ...
      else if (gates[i][0] == "AND3")
        {
          ...
          ss << relabel(gates[i][5], wirenames) << " ";
          ss << relabel(gates[i][7], wirenames) << " ";
          ss << relabel(gates[i][9], wirenames) << " ";
          ss << "AND";
          ngates.push_back(ss.str());
        }
      else if (gates[i][0] == "AND4")
        {
          stringstream ss;
          ss << "4 1 ";
          ss << relabel(gates[i][3], wirenames) << " ";
          ...
          ss << relabel(gates[i][7], wirenames) << " ";
          ss << relabel(gates[i][9], wirenames) << " ";
          ss << relabel(gates[i][11], wirenames) << " ";
          ss << "AND";
          ngates.push_back(ss.str());
        }
