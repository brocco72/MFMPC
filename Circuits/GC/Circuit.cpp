...
istream &operator>>(istream &s, Circuit &C)
{
  ...
  C.GateT.resize(nG);
  C.GateI.resize(nG, vector<unsigned int>());
  C.GateO.resize(nG);
  ...
  for (unsigned int i= 0; i < C.GateT.size(); i++)
    {
      ...
      else if (C.GateT[i] == AND) {
          int fanin = C.GateI[i].size();
          s << fanin << " 1 ";
...
unsigned int Circuit::cnt_numI(unsigned int i) const
{
    unsigned int nI= GateI[i].size();
    if (GateT[i] == INV || GateT[i] == EQW)
    {
        nI= 1;
    }
    else if (GateT[i] == EQ)
...
{
  for (unsigned int i= 0; i < this->cnt_numI(j); i++)
    {
...
void Circuit::swap_gate(unsigned int i, unsigned int j)
{
  ...
  GateO[i]= GateO[j];
  GateO[j]= t;
  vector<unsigned int> tv = GateI[i];
  GateI[i] = GateI[j];
  GateI[j] = tv;
}
